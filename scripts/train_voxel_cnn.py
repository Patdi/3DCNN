...
class VoxelManifestDataset(Dataset):
    """Dataset backed by a manifest generated by prepare_dataset.py."""
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        row = self.rows[idx]
        shard = self._load_shard(Path(row["sample_path"]))
        sample = shard[int(row["sample_index"])].astype(np.float32)
